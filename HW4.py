...
# %%
# AV method
def QMC_AV(weights, method, sequence, npts, d, T=1, r=0.05, sigma=0.3, S0=100, K=100):
    res = np.zeros(npts)
    dt = T/d
    t = np.arange(1, d + 1) * dt
    if sequence == "Faure":
...
def QMC_CV(weights, method, sequence, npts, d=64, T=1, r=0.05, sigma=0.3, S0=100, K=100):
    ...
    if method == "BB":
        for i in range(npts):
            ...
            res[i, 1] = np.exp(-r * T) * max(np.prod(S**weights) - K, 0)
        ...
        for i in range(npts):
            ...
            res[i, 0] = np.exp(-r * T) * max(np.average(S, weights=weights) - K, 0)
            res[i, 1] = np.exp(-r * T) * max(np.prod(S**weights) - K, 0)
    b_star = np.sum((res[:, 1] - res[:, 1].mean()) * (res[:, 0] - res[:, 0].mean())) / np.sum((res[:, 1] - res[:, 1].mean())**2)
